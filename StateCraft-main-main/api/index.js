...
const DB_FILE = path.join(process.cwd(), 'data', 'statecraft_db.json');
const DEFAULT_DB_FILE = path.join(process.cwd(), 'data', 'default_states.json');
const MARKET_CONFIG_FILE = path.join(process.cwd(), 'data', 'market_config.json');
...
module.exports = async function handler(req, res) {
  ...
  let reqUrl = matchedPath && matchedPath.startsWith('/api') ? matchedPath : req.url;
  ...
  if (req.method === 'POST') {
    ...
    if (pathname === '/api/market/config') {
      ...
      try {
        const target = getActiveMarketFile();
        fs.writeFileSync(target, JSON.stringify(newItems, null, 2), 'utf-8');
